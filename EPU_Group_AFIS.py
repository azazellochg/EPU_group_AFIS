...
Last updated 29.03.2024 by Grigory Sharov
"""
...
def main(xml_dir=os.getcwd(), n_clusters=1, apix=1.00,
         ...
         ftype='mrc', output_fn='movies.star',
         algorithm='kmeans', quiet=False):
    metadata_fns = list(iglob(os.path.join(xml_dir,
                                           "Images-Disc*/GridSquare_*/Data/FoilHole_*_Data_*.xml")))
    ...
    for metadata_fn in metadata_fns:
        try:
            ...
            print('Error parsing XML file:', metadata_fn)
            continue
        ...
        if len(beam_shift) == 0:
            raise ValueError(f'No BeamShift found in {metadata_fn}')
        elif len(beam_shift[0]) != 2:
            raise ValueError(f'Improper BeamShift found in {metadata_fn}')
    ...
    with open(output_fn, 'w') as f:
        ...
        for metadata_fn, optics_group in entries:
            ...
            movie_fn = os.path.join(dir, root) + ext

            f.write(f"{movie_fn}\t{optics_group}\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algorithm', type=str, default='kmeans',
                        choices=['kmeans', 'hac'],
                        help='Clustering algorithm to use. {kmeans, hac} [kmeans]')
    ...
    parser.add_argument('--voltage', type=float, default=300.0,
                        help='Voltage of microscope in kV. [300.0]')
